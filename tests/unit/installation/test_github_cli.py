...
import helix_mcp.installation.github_cli as github_cli
...
class FakeRunner:
    ...
    def __call__(self, command: list[str], **kwargs: object):
        ...
        stdout = (
            f"gh version {GITHUB_CLI_VERSION} (test)\n"
...
def _tar_archive(member: str, binary: bytes) -> bytes:
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz") as archive:
        metadata = tarfile.TarInfo(member)
        metadata.mode = 0o755
        metadata.size = len(binary)
        archive.addfile(metadata, io.BytesIO(binary))
    return output.getvalue()


def _zip_archive(member: str, binary: bytes) -> bytes:
    output = io.BytesIO()
...
def test_dry_run_plan_does_not_create_the_tool_directory(
    tmp_path: Path,
) -> None:
    plan = managed_github_cli_plan(
        tmp_path,
        sys_platform="linux",
        machine="x86_64",
    )

    assert plan.version == GITHUB_CLI_VERSION
    assert plan.installed is False
    assert plan.downloaded is False
    assert plan.command == (
        tmp_path / "tools" / "github-cli" / GITHUB_CLI_VERSION / "bin" / "gh"
    )
...
def test_managed_cli_is_installed_reused_and_isolated(
    ...
    metadata = json.loads(
        (installed.command.parent.parent / "installation.json").read_text()
    )
    ...
    def handle(request: httpx.Request) -> httpx.Response:
        if request.url.host == "github.com":
            return httpx.Response(
                302,
                headers={"Location": "https://example.test/gh.tar.gz"},
            )
...
def test_download_requires_https_even_for_an_allowed_host(
    tmp_path: Path,
) -> None:
    requested = False
...
def test_github_environment_removes_all_authentication(
    tmp_path: Path,
) -> None:
