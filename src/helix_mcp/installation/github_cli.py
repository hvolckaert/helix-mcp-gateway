...
class GitHubCliAsset:
    ...
    def url(self) -> str:
        return (
            f"{_GITHUB_CLI_RELEASE_ROOT}/v{GITHUB_CLI_VERSION}/"
            f"{self.archive_name}"
        )
...
class GitHubCliTransport(Protocol):
    """Transport used to retrieve one pinned public GitHub CLI asset."""
    ...
    def download(
        ...
        *,
        ...
        try:
                ...
                for _ in range(10):
                    ...
                    if (
                        ...
                        raise GitHubCliError(
                            "GitHub CLI download left trusted GitHub hosts"
                        )
                    with client.stream("GET", current_url) as response:
                        if response.is_redirect:
                            ...
                            if not location:
                                raise GitHubCliError(
                                    "GitHub CLI download redirect is invalid"
                                )
                        ...
                        return
                raise GitHubCliError(
                    "GitHub CLI download has too many redirects"
                )
            ...
            destination.unlink(missing_ok=True)
            raise GitHubCliError(
                "GitHub CLI archive could not be downloaded"
            ) from None
...
def github_cli_asset(
    *,
    ...
    if selected_platform.startswith("linux"):
        ...
        raise GitHubCliError(
            "managed GitHub CLI is unavailable for this platform"
        )
    selected_machine = (machine or platform.machine()).casefold()
    ...
    architecture = architecture_aliases.get(selected_machine)
    if architecture is None:
        raise GitHubCliError(
            "managed GitHub CLI is unavailable for this architecture"
        )
...
def managed_github_cli_path(
    workspace: str | Path,
    *,
    asset: GitHubCliAsset | None = None,
) -> Path:
    """Return the versioned managed GitHub CLI command path."""
...
def managed_github_cli_plan(
    ...
    *,
    ...
    """Describe the managed command without downloading or executing it."""
...
def ensure_managed_github_cli(
    ...
    *,
    ...
    _ensure_directory(resolved_workspace)
    _ensure_directory(resolved_workspace / "tools")
    _ensure_directory(resolved_workspace / "tools" / "github-cli")
    _ensure_directory(version_root)
    _ensure_directory(command.parent)
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            raise GitHubCliError(
                "managed GitHub CLI could not be installed"
            ) from None
    ...
    try:
        if path.exists():
            if path.is_symlink() or not path.is_dir():
                raise GitHubCliError(
                    "managed GitHub CLI directory is not safe"
                )
        else:
        ...
        raise GitHubCliError(
            "managed GitHub CLI directory could not be prepared"
        ) from None
...
def _extract_binary(
    ...
    try:
        if asset.archive_kind == "tar.gz":
            with tarfile.open(archive, mode="r:gz") as bundle:
                tar_members = [
                    ...
                    if member.name == asset.binary_member
                ]
                if (
                    len(tar_members) != 1
                    or not tar_members[0].isfile()
                    or tar_members[0].size > _MAX_BINARY_BYTES
                ):
                    raise GitHubCliError(
                        "GitHub CLI archive has an invalid executable"
                    )
                source = bundle.extractfile(tar_members[0])
                if source is None:
                    raise GitHubCliError(
                        "GitHub CLI archive has an invalid executable"
                    )
            ...
            with zipfile.ZipFile(archive) as bundle:
                zip_members = [
                    ...
                    if member.filename == asset.binary_member
                ]
                mode = (
                    zip_members[0].external_attr >> 16
                    if len(zip_members) == 1
                    else 0
                )
                if (
                    len(zip_members) != 1
                    or zip_members[0].is_dir()
                    or zip_members[0].file_size > _MAX_BINARY_BYTES
                    or stat.S_IFMT(mode) == stat.S_IFLNK
                ):
                    raise GitHubCliError(
                        "GitHub CLI archive has an invalid executable"
                    )
                with bundle.open(zip_members[0]) as source:
                    _copy_binary(source, destination)
    except GitHubCliError:
        destination.unlink(missing_ok=True)
        raise
        ...
        destination.unlink(missing_ok=True)
        raise GitHubCliError(
            "GitHub CLI archive could not be extracted"
        ) from None
    ...
    with destination.open("xb") as stream:
        for block in response.iter_bytes():
            ...
            if written > _MAX_ARCHIVE_BYTES:
                raise GitHubCliError(
                    "GitHub CLI archive exceeds the size limit"
                )
            stream.write(block)
...
def _copy_binary(source: _BinaryReader, destination: Path) -> None:
    try:
        with destination.open("xb") as output:
            ...
            while True:
                ...
                if copied > _MAX_BINARY_BYTES:
                    raise GitHubCliError(
                        "GitHub CLI executable exceeds the size limit"
                    )
                output.write(block)
    except GitHubCliError:
        destination.unlink(missing_ok=True)
        ...
        destination.unlink(missing_ok=True)
        raise GitHubCliError(
            "GitHub CLI executable could not be extracted"
        ) from None
...
def _run(
    ...
    *,
    ...
    try:
        ...
        raise GitHubCliError(
            "managed GitHub CLI could not be executed"
        ) from None
...
def _write_metadata(
    ...
    try:
        ...
        raise GitHubCliError(
            "managed GitHub CLI metadata could not be installed"
        ) from None
    finally:
